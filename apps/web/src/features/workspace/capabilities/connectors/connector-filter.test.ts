...
describe('the landing tab is All, unconditionally', () => {
  ...
  // tabs — it decides whether All exists at all. On a self-host with no
  // Pipedream credentials the catalogue it shows answers `501`, so it is
  // removed and Connected is the only tab left to land on. See
  ...
  test('the default scope is a constant, not derived from the project', () => {
    ...
    // landing tab did not move: an absent or unrecognised param is All, for
    // every project, before any query resolves — unless the deployment has
    // no catalogue to land on, in which case it falls back to Connected.
