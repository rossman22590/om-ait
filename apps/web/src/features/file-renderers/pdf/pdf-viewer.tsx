...
function PDFViewerFallbackShell({
  ...
  return (
    <div
      ...
      className={cn(
        'bg-background flex h-full max-h-full min-h-0 w-full flex-col overflow-hidden',
        className,
      )}
    >
      {showToolbar ? (
...
function PDFViewerInner({
  ...
  return (
    ...
    >
      ...
      <div
        ref={viewerShellRef}
        className="bg-muted/30 relative flex min-h-0 flex-1 overflow-hidden"
      >
        {isLoading ? (
  ...
  if (engineError) {
    return (
      <div
        data-slot="pdf-viewer"
        className={cn(
          'bg-background text-muted-foreground grid h-full w-full place-items-center p-6 text-sm',
          className,
        )}
      >
