...
## When paid credits arrive

Credits follow settled payments. A payment that is accepted but not yet
settled adds nothing to the balance until it settles.

- **Top-ups paid by bank debit** — card payments settle at once. A bank debit
  (for example ACH) can take several business days; the credits arrive when
  it settles. A failed debit adds nothing.
- **Auto top-up** — the same rule applies. While one bank-debit top-up is
  still settling, auto top-up does not start a second charge.
- **Adding seats mid-cycle** — each added seat is charged now, prorated for the
  rest of the billing cycle. Once that charge is paid, the balance grows by the
  same prorated share of the seat's monthly allowance.
- **Removing seats mid-cycle** — the lower seat count is billed from your next
  renewal. The unused part of the current cycle is not refunded, and credits
  already granted stay in the balance.
- **Upgrading a plan mid-cycle** — the price difference is charged now, and
  the upgrade applies only when that payment succeeds. The balance then grows
  by the prorated share of the new plan's monthly allowance. Your next renewal
  grants the full allowance.
