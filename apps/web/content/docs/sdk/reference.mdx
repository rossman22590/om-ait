...
| `call(tool, args?, options?)` | call one `<connector>.<action>` tool through the server-side gateway. `options.account` names which account to run as — see below. |
| `accounts(slug)` | the accounts this caller may call `slug` as, default first — what `options.account` matches against |
...
| `setAuthorizationStrategy(slug, strategy)` | @deprecated. Still calls the route; the route is a no-op that answers `{ok:true, deprecated:true}` and changes nothing. `owner_type` on each connection is the whole access rule now. |
...
A connector defines the tool, provider app, and policies. `connections`
stores its connected **accounts** — a connector can hold several side by side.
A session can pin one for every call with
`connector_bindings: { alias: { connection_id } }`, or a single call can name
one with `call(tool, args, { account })`. Credentials stay encrypted and
resolve per request.
...
#### Accounts and the call-time gate

Each connection has an `owner_type`: `project` (shared — reachable by anyone
the connector is granted to, human or service account) or `member` (private —
reachable only by its own owner, and only in a private session; never by a
service account). `p.connectors.accounts(slug)` lists every account this
caller may reach, default first:

```ts
const accounts = await kortix.project(projectId).connectors.accounts('stripe-read');
// [{ connection_id, label, owner_type, is_default }, ...]

await kortix.project(projectId).connectors.call(
  'stripe-read.get_balance',
  {},
  { account: 'My Stripe' }, // a connection id, a label, or the words `me` / `project`
);
```

A call that names no account resolves to the caller's own default private
account first, then the project's default shared account. A named account
that does not resolve is **denied** with `connector_not_connected` — never
silently substituted — and the denial lists `requested_account` and
`available_accounts`. Every successful call echoes which account it ran as:
`result.account = { connection_id, label, owner_type }`.

There is no session-create-time connector gate any more. A session used to be
refused `409 CONNECTOR_CONNECTION_REQUIRED` when a required connector had no
usable connection; that refusal is gone (`require_connectors` is inert — see
[`p.sessions`](#psessions--and-the-session-handle)). The gate moved to the
call: a denied `connector_not_connected` carries `connect_url`, a hosted
authorization link, whenever one can be minted for the caller — the whole
remedy is one click, surfaced by the transcript's connect button.
...
backend-origin secret narrowing. `require_connectors` is deprecated and
inert: the field is still accepted (an old client is never rejected for
sending it), but nothing reads it — a session is never refused at create time
for a connector with no usable account. See [Accounts and the call-time
gate](#accounts-and-the-call-time-gate).
...
`kortix.providerConnections` manages your ChatGPT subscription and provider API keys across projects. Use a signed-in user's JWT. Account PATs and sandbox tokens cannot manage these credentials.

Save up to ten named connections per provider with `saveApiKey(provider, key, { create: true, label })` or `startOAuth(provider, { create: true, label })`. Pass `{ connection_id }` to update an existing connection. `remove(provider, connectionId)` removes one connection; `remove(provider)` removes them all.

Choose a saved connection with `setProject(projectId, provider, true, { connection_id })`, or enable your personal pool with `{ pool: true }`. Each session retains one pool member across requests and API replicas. Removing that member allows a new selection. Requests without a session select a member per request. Pools never include another person's credentials. Provider limits and authentication errors remain visible; rejected requests are not retried against another pool member.

| Method | Effect |
| --- | --- |
| `list()` | List supported providers and your connection metadata |
| `saveApiKey(provider, apiKey)` | Save or replace your encrypted provider key |
| `startOAuth('codex')` | Start ChatGPT device authorization |
| `pollOAuth('codex', flowId)` | Complete authorization and save your connection |
| `listProject(projectId)` | List your enabled connections in a project |
| `setProject(projectId, provider, enabled)` | Enable or disable your connection in a project |
| `remove(provider)` | Disconnect and remove all project bindings |

Personal providers require the LLM gateway. Sessions use the launching user's authenticated identity, including when another participant sends a message in that session. Enabling a connection grants its use to those sessions. Without a personal binding, existing project credentials apply. Removing a connection removes its project bindings immediately; requests already in flight can finish.

Manage saved accounts in **Preferences → My providers**. Under **Project → Models → Providers**, each provider appears once with a status and **Connect** or **Manage** action. The dialog offers **Use my account** by default for supported providers. OpenAI supports **ChatGPT subscription** and **API key**. Select **Share with this project** explicitly to save shared credentials. Confirm with **Save connection**, **Use this account**, or **Use project connection**. Changing a choice or cancelling performs no write. **Stop using my account** removes this project binding while preserving the saved account for other projects. Project members can connect their own accounts without permission to change shared credentials. Tokens remain on the API and never enter sandbox environment variables.
