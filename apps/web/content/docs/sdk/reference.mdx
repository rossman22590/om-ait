...
| method | what |
| --- | --- |
| `listMine()` | every pending, unexpired invite addressed to your email, with the workspace and any projects it grants |
...
| `replaceProjectRepository({ project_id, repo_url, expected_repo_url, github_token })` | replace an existing project's GitHub repository. Use either `github_token` with Contents read/write access, or `installation_id` plus temporary `github_user_token` to verify organization admin access and grant only this repository through the GitHub App. `copy_shared_secrets_from_project_id` plus `copy_shared_secret_identifiers` copies selected shared runtime secrets in the same transaction. A stale `expected_repo_url` returns 409. The old repository remains on GitHub. New sessions use the replacement repository; existing sessions retain their checkout. |
...
| `uploadAttachment(content, input)` | stage bytes for `input.connector`; returns `ref` (`{ $kortix_attachment }`) to place in a later call's args — see [Attach files](/docs/connect/connectors#attach-files) |
...
| `connections.setDefault(connectionId)` | make it the default for its owner scope |
| `connections.rename(connectionId, label)` | change the label only; the account stays authorized. Each connection carries `connected_as`, the identity it was authorized as. |
...
The gateway moves to the next fallback model only when the current model
failed before it returned any output:

- `transient`: a network error, a timeout, any `5xx`, or `402`, `403`, `429`.
- `any-error`: every failure, including a `400`.

A failure after output started stays with that model. A request on your own
provider key (BYOK) never falls back to a Kortix-billed model. Each fallback
model gets its own generation defaults.
