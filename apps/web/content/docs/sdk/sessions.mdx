...
several granted account secrets for one provider. Create resources through
`kortix.accounts.secretResources.create(accountId, input)` and grant use with
`.grant(accountId, secretId, userId)`. Reads return metadata and grants, never
