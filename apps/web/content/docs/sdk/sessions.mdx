...
- `s.attachments.upload(file, options?)` — save a private file before startup when `session_transcript_history` is enabled. Maximum 50 MiB. Returns a stable URL for a prompt file part.
...
- `s.transcriptSync(options?)` — the transcript the server saves at the end of every turn, as OpenCode message envelopes. It answers while the sandbox is stopped or still waking, without starting it. Pass `limit` (at most 500) for the newest window, and a window's `next_cursor` as `before` to read the one older than it. `source` is `mirror`; `complete` is true only when the window holds every saved message and the saved copy reaches the session's first message.
