...
- `s.attachments.upload(file, options?)` — save a private file before startup when `session_transcript_history` is enabled. Maximum 25 MiB. Returns a stable URL for a prompt file part.
