...
For the whole product in your terminal — the session list, the transcript and
composer, a shell in the sandbox, and the Files, Review, Apps, Customize and
Account screens — run the experimental terminal client:

```sh
kortix tui
```

It signs in with the host you are already on. See [the TUI page](/docs/tui).
...
Non-interactive credentials for the account. Reads need `token.read`. Every
account member may mint and revoke their own personal keys
(`token.personal.create`, `token.personal.revoke`); a personal key acts as you
and never carries more than your own roles. Service accounts and other people's
keys need `token.create` / `token.revoke` (owner and admin).
...
| `kortix projects cli-tokens new [--name <name>] [--expires-at <iso>]` | Mint a project-scoped CLI token. The secret prints once. |
...
`project.credentials.issue`. A session-bound token can neither mint nor
revoke project tokens (`403`), whether or not the project declares agents. A
project CLI token follows the account's PAT policy: when the account requires
an expiry, pass `--expires-at`; an expiry beyond the maximum lifetime returns
`400`.
...
Each run:
- Parses and validates `kortix.yaml`, merged with every file it lists under [`imports:`](/docs/project/manifest#imports) (skip with `--no-verify`).
...
| `kortix tui [--host <name>] [--project <id>] [--session <id>] [--install] [--uninstall]` | **Experimental.** Open the Kortix terminal client: sessions, transcript, composer, sandbox shell, Files, Review, Apps, Customize and Account. The TUI is a separate ~80 MB binary; the first run asks, then installs the version-matched `kortix-tui` into `~/.kortix/tui/<version>/`. `--install` does it without a prompt, `--uninstall` removes it. Uses the active host unless `--host` names another; with no host logged in it opens its own login screen. `?` prints every binding. See [/docs/tui](/docs/tui). |
...
| `kortix sessions log [<id>] [--limit <n>] [--json]` | Print recent messages, read-only. A running session is read live; a stopped one is read from the transcript saved at the end of every turn, so it is never woken just to be read. The source is noted on stderr, so `--json` output is unchanged. Aliases: `messages`, `history`. |
| `kortix sessions attachments <id> [--download <attachment-id>\|--all] [--out <dir>] [--json]` | List a session's stored files — what users attached, and copies of the files the agent showed — and download them. Works while the session is stopped: neither the list nor the bytes need its sandbox. Downloads never overwrite; a name already taken becomes `name (1).ext`. |
...
| `kortix sessions digest [--since <7d>] [--json]` | Compact multi-session review. A stopped session's transcript comes from its saved copy and is marked `saved` (and `partial` when that copy does not reach the first message). Aliases: `review`, `summary`. |
...
`sessions attachments` reads the session's private file store, not its
workspace. Files land there when the project has **Session Transcript History**
enabled (Settings → Feature flags): uploads at send time, and every file the
agent showed at the end of the turn. Files stored while the flag was on stay
downloadable after it is turned off.
...
| `kortix agents scope <agent> [--secrets all\|none\|A,B] [--connectors all\|none\|a,b] [--apps all\|none\|a,b] [--require-connector <slug>]` | Replace which secrets, connectors and Kortix Apps the agent may use. `--apps` takes App slugs and needs a v2 `kortix.yaml`; the agent also needs `project.app.read`. `--require-connector` is repeatable and must resolve before a session starts. |
...
| `kortix connectors accounts <slug> [--default <label\|id>] [--json]` | List the accounts your calls may run as, with the identity each is connected as. `--default` pins one. |
| `kortix connectors connections ls [--all] [--json]` | List connections with label, `CONNECTED AS`, owner, status, and default. |
| `kortix connectors connections rename <id> <label…>` | Rename a connection. Only the label changes; no re-authorization. |
...
| `kortix connectors call <connector>.<action> '<json-args>'` | Run a tool. A governed call returns its authenticated `approval_url` immediately. The server resumes the session after one approve or deny decision. Pass large args as `@file.json` or `-` (stdin). |
| `kortix connectors call <connector>.<action> @args.json --attach <file>` | Stage a file and append its reference to the action's attachments array. The gateway writes the bytes into the provider's attachment item. `--attach` repeats; `--attach-path <a.b.c>` names the array. See [Attach files](/docs/connect/connectors#attach-files). |
| `kortix connectors upload <file> --connector <slug>` | Stage one file. Prints `ref`, `{"$kortix_attachment":"<id>"}`, to place in call args. |
...
carries Kortix permissions in `kortix.yaml`, and a session can only do what both
...
`session_sequence`. `--all` follows every continuation cursor. The table shows
each row's title (`EVENT`, for example "Revoked LLM gateway key") next to its
`ACTION` (`gateway.key.revoke`); `--action` filters by action prefix. Every
action and title: [Audit log actions](/docs/audit-actions).
