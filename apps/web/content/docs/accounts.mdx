...
Which role counts depends on the project flag `agent_principal`. Off: the role of the person who started the session. On: the role bound to the agent's own service account — its **ceiling** — or the default agent ceiling when none is bound. Three permissions (`project.members.manage`, `project.delete`, `project.credentials.issue`) are human-only and never reach an agent. Admins pick an agent as the principal of a project role in the Access hub, under **Agents**. See [Agent permissions](/docs/project/permissions).
