...
> **Experimental.** `kortix tui` ships in the CLI binary, but its keys, screens
...
That is the whole setup. `kortix tui` is part of the [CLI](/docs/cli) binary and
signs in with the host you are already on, so there is nothing else to install —
the renderer and its native library are compiled into the binary.
...
`apps/tui` in the [Kortix repo](https://github.com/kortix-ai/suna), wired into
the CLI by `apps/cli/src/commands/tui.ts`. `SPEC.md`
