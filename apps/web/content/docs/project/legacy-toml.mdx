...
Kortix still supports v1 manifests. The platform resolves a project's manifest in this order: `kortix.yaml`, then `kortix.yml`, then `kortix.toml`. The current starter creates one v2 `kortix.yaml`. A v1 project keeps working with no forced upgrade.
