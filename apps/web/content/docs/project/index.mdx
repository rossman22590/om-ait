...
Every project has a manifest at its repo root, `kortix.yaml` by default. A large manifest can [split across files with `imports:`](/docs/project/manifest#imports). The
platform reads these top-level keys:

| Key | Configures |
|---|---|
| `default_agent` | which agent runs by default |
| `agents` | per-agent grants: connectors, secrets, skills, Kortix permissions |
