...
The check reads `kortix.yaml` at the merge base and at the change request's
head. When it cannot read either one, it does not merge. It answers `503`
with `code: "CR_GOVERNANCE_UNVERIFIED"` and a `Retry-After` header. Retry the
merge.
