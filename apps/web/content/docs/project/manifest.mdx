...
agents:
  ...
  release-bot:
    sandbox: ml
    connectors: [github]
    kortix_permissions: [project.gitops.push]
...
| `imports`                                           | no       | other YAML files or directories whose `triggers`, `connectors`, `agents`, and `apps` merge into this manifest. See [`imports:`](#imports) |
| `runtime`                                           | no       | `"opencode"` (the default) or `"pi"` — the agent harness a session boots inside its sandbox; `pi` runs pi-agent-core in-process in the sandbox daemon and reads the same `.kortix/opencode/agents/*.md` and skills. The project's `pi_harness` feature flag selects pi without this line. |
...
| `agents`                                            | yes      | name-to-block governance map; must not be empty. With `imports`, the map may live in an imported file |

## `imports:`

Split one manifest across several YAML files. Use it when `kortix.yaml` outgrows one screen: 30 triggers with multi-line prompts, a connector per team, one file per agent group.
...
default_agent: kortix
imports:
  - .kortix/triggers/        # a directory: every .yaml and .yml file below it
...
triggers:
  - slug: weekly-report
    name: Cross-Sector Weekly Report
    type: cron
    agent: galileo
    cron: "0 15 * * 0"
    prompt: |-
...
The platform reads the root file, resolves every import, and works on the merged document. Validation, trigger sweeps, agent grants, and connector sync all see one manifest. A trigger in one file can name an agent declared in another.

**Rules**

1. A path is relative to the repository root. `..`, absolute paths, and globs are rejected.
2. A directory import takes every `.yaml` and `.yml` file below it, at any depth, sorted by path. Other files are skipped.
3. An imported file declares `triggers`, `connectors`, `agents`, `apps`, and `imports`. Every other key (`kortix_version`, `default_agent`, `runtime`, `project`, `env`, `opencode`, `sandbox`, `policy`, `policies`) stays in the root file. An imported file that sets one fails validation.
4. An imported file can import more files. The limit is 8 levels and 200 files. A cycle is an error. A file reached twice merges once.
5. One name, one file. A trigger slug, connector slug, agent name, or app name declared in two files is an error that names both files. Nothing overrides silently.
6. Entries merge in order: the importing file first, then its imports in the order listed.

A broken import fails the whole manifest, the same way a YAML syntax error does. `kortix validate`, `kortix ship`, and the change request merge check all report it before it reaches `main`:

```text
imports: triggers "weekly-report" is declared in both .kortix/triggers/a.yaml and .kortix/triggers/b.yaml — one name, one file
```

**Edits go to the file that declares the entry.** When the dashboard, the API, or `kortix triggers enable|disable|rm` changes a trigger, connector, agent, or app that an imported file declares, only that file is written. The root file is not rewritten. A new entry is written to the root file. Move it into an imported file by hand if you want it there.

`imports` requires `kortix.yaml`. A `kortix.toml` manifest ignores the key.
...
This block is the **second of the two bindings**: a principal gets roles from Kortix, and an agent additionally carries the Kortix permissions declared here. A session can only do what both allow. See [One vocabulary, two bindings](/docs/accounts#one-vocabulary-two-bindings).
...
| `connectors_required` | `none`  | Deprecated: parsed and ignored. Nothing gates sandbox startup on a connector any more; a call to a connector with nothing connected returns `connector_not_connected` with a connect link. Must still be a subset of `connectors` when present. |
| `secrets`             | `none`  | Which project secrets this agent receives as sandbox env vars. Renamed from v1's `env`.                                                                                                                   |
| `kortix_permissions`  | `none`  | Which Kortix permissions (project actions) this agent may exercise, through the CLI, the API, or git. `all` means everything the launching user's roles allow — an agent can never exceed its launcher. `kortix_cli` is the deprecated spelling: still accepted, with a validation warning; setting both to different values is an error. |
| `skills`              | `none`  | Which skills this agent may load.                                                                                                                                                                         |
| `apps`                | `none`  | Which Kortix App slugs this agent may open when the App is `restricted` or `private`. `all` means every App in the project. Enforced while the project's `agent_principal` flag is on. See [Agents and Apps](/docs/feature-flags/apps#agents-and-apps). |
...
v2 is deny-by-default: an omitted `connectors`, `secrets`, `kortix_permissions`, `skills`, or `apps` on a declared agent resolves to `none`. Grant every permission an agent needs explicitly, as the starter's `kortix` agent does above. A project migrating from v1 must re-grant everything by hand — v1 defaults `env` (secrets) to `all` when omitted, the opposite of v2.
...
Dashboard edits are a read-modify-write on the file that declares the entry: `kortix.yaml`, or an [imported file](#imports). To keep diffs clean across UI and in-session edits:
