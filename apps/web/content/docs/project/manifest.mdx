...
| `kortix_permissions`  | `none`  | Which Kortix permissions (project actions) this agent may exercise, through the CLI, the API, or git. `all` means every project permission, still capped by the agent's ceiling role and never including the human-only permissions. `kortix_cli` is the deprecated spelling: still accepted, with a validation warning; setting both to different values is an error. |
| `skills`              | `none`  | Which skills this agent may load.                                                                                                                                                                         |
| `apps`                | `none`  | Which Kortix App slugs this agent may open when the App is `restricted` or `private`. `all` means every App in the project. See [Agents and Apps](/docs/feature-flags/apps#agents-and-apps). |
