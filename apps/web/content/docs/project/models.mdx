...
When the `pooled_provider_secrets` flag and LLM gateway are enabled, open
**Models → Providers** to add multiple named API keys for one provider. Each
key has its own member access and can be rotated or deleted separately. Select
the keys a session may use in its session settings. An existing project key
remains available when the session has no explicit key selection.
Google Gemini API keys use Google's OpenAI-compatible endpoint when the LLM
gateway is on. ChatGPT Plus/Pro uses OAuth. Under the flag, each member can
connect several named ChatGPT accounts. An account is private when connected.
Its owner can grant another account member access. A session can select one or
more granted accounts. The gateway tries another selected account after a
pre-output rate limit. Without a session selection, a member's newest personal
account is used. The older project login remains available when that member
has no personal account.
