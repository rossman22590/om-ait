...
to use account secret resources. Open **Customize → Models → Providers** and
...
Open a key's **⋯ → Manage access** to choose account members who may use it.
Access applies across projects in that account. Account membership, project
permissions, the agent's secret permissions, and session selection still apply.
...
then select up to ten keys. For an existing session, choose **Save key selection**.
...
separate named account. It starts private to the member who connected it.
Use **Manage access** to share it with other account members.

A session can select multiple granted ChatGPT accounts. Without a selection,
the member's newest personal connection is the default. The legacy project
login remains available when that member has no personal connection. A shared
