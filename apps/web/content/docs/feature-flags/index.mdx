...
- **`pi_worker`** — compile a pi-based worker runtime and the OpenCode
  compiled-boot artifacts on every push. Sessions boot on the split
  worker/environment topology only when the manifest also sets `runtime: pi`.
- **`pi_harness`** — run this project's sessions on the pi agent harness
  ...
  decides — `runtime: pi` still boots pi, anything else boots OpenCode. Same
  repo layout, agents and skills either way; a running session keeps its
  harness until it is restarted or resumed.
### Graduated flags

A graduated feature is on for every project and has no switch.

- **Agents as principals** is standard behavior. An agent session acts as the
  agent itself — its Kortix permissions within its ceiling role, minus the
  human-only permissions, whoever runs it. It is not listed on
  **Settings → Feature flags** and there is nothing to turn on. See
  [Agent permissions](/docs/project/permissions). One project can be put back
  on the old launcher-role model for a single release; ask support, who set it
  with `PATCH /v1/projects/:id/features`. That switch is removed in the release
  after this one.
