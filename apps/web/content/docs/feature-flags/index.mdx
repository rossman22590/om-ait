...
| `agent_principal` | Agent Principal | Experimental | Off | `behavioral` |
...
- **`agent_principal`** — make each declared agent the acting principal of its
  sessions: its Kortix permissions within its ceiling role, minus the
  human-only permissions, whoever runs it. Off keeps the launcher's role as the
  cap. See [Agent permissions](/docs/project/permissions).
