...
This section applies when the project's `agent_principal` feature flag is on.
With the flag off, the gate and the connector gateway behave as described above.
...
`X-Kortix-App-Authorization` before it forwards the request (with the flag on
or off), so the App never receives it. The App's own `Authorization` header is
forwarded unchanged.
