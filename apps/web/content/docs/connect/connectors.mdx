...
MCP is the recommended provider for a new connector. The Connectors catalogue
lists MCP servers first, and a server that implements the MCP authorization
specification connects in one click — no app to create, no client ID to paste
(see [Connect an MCP server that uses OAuth 2.1](#connect-an-mcp-server-that-uses-oauth-21)).
...
A project connector's credential can also come from a project secret. Bind an
existing secret by its identifier from the **Credential source** row on the
connector's page, or from the secret's own delivery settings on the Secrets
page. The connector then follows the secret: rotating the secret value rotates
what the connector sends, and the connector page names the bound identifier.
...
Open the project. Select **Connectors**, then select the app. Each catalogue or
project connector opens on its own URL. You can bookmark or share that URL.

The connector page keeps the required connection action above the setup steps.
It also links to this guide and to provider documentation when the catalogue
publishes a documentation URL.
...
After authorization, use the connector page tabs:

- **Accounts** manages project or member connections and credentials.
- **Tools** controls connector actions and their approval policy.
- **Settings** controls authorization ownership and connector lifecycle.
- **Advanced** shows transport, endpoint, authentication, access scope, and
  request headers.

## Connect an MCP server that uses OAuth 2.1

Kortix implements the MCP authorization specification. Open the connector's
page and select **Add credential**. Kortix probes the server first: when the
server requires OAuth 2.0, the dialog opens on the **OAuth 2.0** tab; when it
uses a plain token, only the credential field appears (with a **Use OAuth 2.0
instead** action for servers that require OAuth without advertising it). The
probe reads the server's metadata:
