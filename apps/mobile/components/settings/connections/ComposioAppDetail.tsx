...
export function ComposioAppDetailContent({
  ...
  const { data: connections, isLoading: connectionsLoading } = useComposioConnections();
  ...
  const appConnections = React.useMemo(() => {
    if (!connections || !app) return [];

    const filteredConnections = connections.filter((connection: ComposioConnection) => {
      const isConnected = connection.is_connected || connection.connection_status === 'active';
      return connection.toolkit_slug === app.slug && isConnected;
    });
            ...
            <View className="flex-row items-center justify-start gap-2">
              ...
              >
                <Text className="text-sm font-roobert-semibold text-white">
                  {hasConnections ? t('connections.connect').toUpperCase() : t('connections.setup').toUpperCase()}
                </Text>
              </Pressable>
              {hasConnections && (
                <View className="flex-row items-center justify-start gap-2">
        ...
        <View className="flex-row items-start justify-between pt-4 border-t border-border/20">
          <View className="flex-1">
            <Text className="text-xs font-roobert text-muted-foreground uppercase tracking-wider">
              {t('connections.appDetails.developer')}
            </Text>
          ...
          <View className="flex-1 items-start">
            <Text className="text-xs font-roobert text-muted-foreground uppercase tracking-wider">
              {t('connections.appDetails.tools')}
            </Text>
            <Text className="text-lg font-roobert-semibold text-muted-foreground">
              ...
              <View style={{ paddingHorizontal: 24, paddingVertical: 32 }}>
                <View className="p-6 bg-muted/5 border border-border/30 rounded-xl">
                  <Text className="text-sm font-roobert text-muted-foreground text-center">
                    {t('connections.appDetails.noToolsFound')}
                  </Text>
                </View>
              </View>
            }
              ...
              availableTools.length > 12 ? (
                <View style={{ paddingHorizontal: 24, paddingTop: 8 }}>
                  <View className="p-3 bg-muted/5 border border-border/30 rounded-xl">
                    <Text className="text-sm font-roobert text-muted-foreground text-center">
                      {t('connections.appDetails.toolsAvailableAfterSetup', { count: availableTools.length - 12 })}
                    </Text>
                  </View>
                </View>
              ) : null
