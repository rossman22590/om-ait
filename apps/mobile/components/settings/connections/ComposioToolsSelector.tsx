...
export function ComposioToolsContent({
  ...
  // Load current enabled tools when the agent, connection, or tools change.
  React.useEffect(() => {
    if (!agentId || !connection?.connection_id || !agent || isLoading || allTools.length === 0) {
    ...
    try {
      // Find the composio MCP with matching connection_id
      const composioMcps =
        agent.custom_mcps?.filter(
          (mcp: any) => mcp.type === 'composio' && mcp.config?.connection_id === connection.connection_id
        ) || [];
  ...
  }, [agentId, connection?.connection_id, agent, allTools, isLoading]);
      ...
      },
      {
        onSuccess: (data) => {
          Alert.alert(
            ...
            t('connections.toolsSelector.toolsAddedSuccess', {
        ...
        onError: (error: any) => {
          Alert.alert(
            t('connections.connectionError'),
            error.message || t('connections.toolsSelector.failedToSaveTools')
          );
        },
      }
    );
  }, [agentId, connection.connection_id, selectedTools, updateTools, app.name, onComplete, t, allTools]);
    ...
    () => (
      <View style={{ paddingHorizontal: useBottomSheetFlatList ? 24 : 0, paddingVertical: 32 }}>
        <EmptyState
          ...
          title={
            searchQuery
              ? t('connections.toolsSelector.noToolsFound')
              : t('connections.toolsSelector.noToolsAvailable')
          }
          description={
            searchQuery
              ? t('connections.toolsSelector.tryAdjustingSearch')
              : t('connections.toolsSelector.toolsAppearHere')
          }
        />
      </View>
    ),
        ...
        <View
          ...
          <View className="mt-4 flex-row items-center justify-between">
            <Text className="font-roobert-medium text-sm uppercase tracking-wider text-muted-foreground">
              {t('connections.toolsSelector.selected', {
                count: selectedTools.size,
                total: totalTools,
            ...
            <Pressable
              ...
              <Text className="font-roobert-semibold text-sm text-foreground">
                ...
                filteredTools.every((tool: ComposioTool) => {
                  ...
                  ? t('connections.toolsSelector.deselectAll')
                  : t('connections.toolsSelector.selectAll')}
              </Text>
            </Pressable>
          </View>
        </View>
