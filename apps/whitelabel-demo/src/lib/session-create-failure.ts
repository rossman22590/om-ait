...
export function sessionCreateFailure(err: unknown): SessionCreateFailure {
  ...
  switch (code) {
    ...
    // The connector PRE-FLIGHT is gone. `CONNECTOR_CONNECTION_REQUIRED` and
    // `REQUIRED_CONNECTOR_CONNECTION_UNAVAILABLE` cannot be returned by create
    // or scope any more — `require_connectors` is accepted and ignored on the
    // wire (see `packages/api-contract` `SessionCreateInputSchema`). The gate
    // moved to the connector CALL, which denies `connector_not_connected` with
    // a `connect_url` the agent's own turn surfaces. Neither case is listed
    // here on purpose: an old server that still sent one would fall through to
    // the `default` arm below rather than resurrect dead UI for it.
