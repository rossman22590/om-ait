...
function slashHelp(ctx: SlashCtx): SlashResponse {
  ...
  const advanced: Array<{ cmd: string; desc: string }> = [
    { cmd: `${command} model <id>`, desc: 'Set the channel model directly, e.g. `kortix/morph-glm53-744b` or `anthropic/claude-sonnet-4.6` (`default` to reset).' },
...
async function slashSetModel(ctx: SlashCtx, arg: string): Promise<SlashResponse> {
  ...
  if (/\s/.test(id)) {
    return { response_type: 'ephemeral', text: `\`${escapeMrkdwn(id)}\` doesn't look like a model id. Use \`provider/model\` (e.g. \`anthropic/claude-sonnet-4.6\`) or a managed id (e.g. \`kortix/morph-glm53-744b\` or \`morph-glm53-744b\`).` };
  }
