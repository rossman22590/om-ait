import { mirrorRowsFromOpencodePayload, type MirrorMessage } from './session-transcript-mirror';
...
export async function readTranscriptPages(
  ...
  do {
    ...
    for (const row of mirrorRowsFromOpencodePayload(payload)) {
      const id = String(row.info.id);
      if (!messages.has(id)) messages.set(id, row);
    }
    cursor = response.headers.get('x-next-cursor') || undefined;
    headComplete = !cursor;
    if (cursor && cursors.has(cursor)) throw new Error('Repeated transcript cursor');
  ...
  const rows = [...messages.values()].sort((a, b) => {
    ...
    return created(a) - created(b) || String(a.info.id).localeCompare(String(b.info.id));
  });
