...
test('named credentials and project pools preserve explicit selection in the request', async () => {
  await saveUserProviderApiKey('openai', 'second-key', { create: true, label: 'Work' });
  await startUserProviderOAuth('codex', { create: true, label: 'Personal' });
  await setProjectPersonalProvider('p', 'codex', true, { pool: true });
  await setProjectPersonalProvider('p', 'openai', true, { connection_id: 'chosen' });
  expect(calls.map((c) => c.body)).toEqual([
    { api_key: 'second-key', create: true, label: 'Work' },
    { create: true, label: 'Personal' },
    { enabled: true, pool: true },
    { enabled: true, connection_id: 'chosen' },
  ]);
});

test('deleting one connection preserves the provider and encodes its identifier', async () => {
  await deleteUserProviderConnection('openai', 'connection/one');
  expect(calls.map((c) => [new URL(c.url).pathname, c.method])).toEqual([
    ['/provider-connections/openai/connections/connection%2Fone', 'DELETE'],
  ]);
});
