...
describe('narrationToolName', () => {
  it('strips the MCP server prefix and title-cases', () => {
    expect(narrationToolName('linear/create_issue')).toBe('Create Issue');
    expect(narrationToolName('oc-session_read')).toBe('Session Read');
  });
});
...
describe('contextLabelForTool - plain-language Context row labels, keyed on family', () => {
  ...
  it('leaves every other family on narrationToolName', () => {
    expect(contextLabelForTool('linear/create_issue')).toBe(
      narrationToolName('linear/create_issue'),
    );
...
describe('narrationToolName - MCP tools using `__` separators must not read as a raw identifier', () => {
  it('normalizes `__` the same way `/` is normalized', () => {
    expect(narrationToolName('mcp__linear__create_issue')).toBe('Create Issue');
  });

  it('never leaves double spaces or underscores from an un-normalized `__`', () => {
    const label = narrationToolName('mcp__linear__create_issue');
