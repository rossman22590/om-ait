...
describe('formatMessageDay — recent reads as distance', () => {
  ...
  test('each unit rounds to the nearest whole value, and a rounded-up unit keeps its unit', () => {
    const at = (ms: number) => formatMessageDay(NOW - ms, NOW, utc);
    expect(at(90_000)).toBe('2 minutes ago');
    expect(at(59.6 * 60_000)).toBe('60 minutes ago');
    expect(at(60 * 60_000)).toBe('1 hour ago');
    expect(at(23.6 * 3_600_000)).toBe('24 hours ago');
    expect(at(24 * 3_600_000)).toBe('1 day ago');
    expect(at(6.6 * 86_400_000)).toBe('7 days ago');
  });
