 ...
 *
 * Named `narrationToolName`, not `humanizeToolName`: the SDK already exports a
 * `humanizeToolName` (tool-registry) that keeps the server segment
 * (`linear/create_issue` → "Linear/create Issue"). This one names only the
 * leaf, which is what a narrated sentence wants.
 *
 ...
 * `linear/create_issue`.
 */
export function narrationToolName(toolName: string): string {
 ...
 * earn an override; every other one keeps `narrationToolName`, where the tool's
...
export function contextLabelForTool(toolName: string): string {
  ...
  return label ?? narrationToolName(toolName);
}
...
export function narrateStep(family: StepFamily, parts: ToolPart[]): string {
  ...
  switch (family) {
    ...
    case 'other': {
      if (n === 1) return `Used ${narrationToolName(parts[0].tool)}`;
      ...
      const names = Array.from(new Set(parts.map((p) => narrationToolName(p.tool))));
...
export function narrateFailedStep(family: StepFamily, parts: ToolPart[]): string {
  ...
  switch (family) {
    ...
    case 'other': {
      ...
      const names = Array.from(new Set(source.map((p) => narrationToolName(p.tool))));
