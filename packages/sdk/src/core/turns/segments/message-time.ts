...
/**
 * The runtime zone's UTC offset at an instant, in milliseconds, with second
 * precision. A day that crosses a DST change is 23 or 25 wall-clock hours, and
 * `relativePast` counts days in wall-clock terms so "1 day ago" still means
 * "same time yesterday".
 */
...
/**
 * `date` → "N minutes ago" / "N hours ago" / "N days ago", for a `date` at
 * least one minute and under one week before `now`.
 *
 * This is `date-fns`' `formatDistanceStrict(date, now, { addSuffix: true })`
 * for the en-US locale, restricted to the range `formatMessageDay` calls it
 * with: the same unit thresholds, the same `Math.round`, and the same DST
 * normalisation for days. It lives here so the SDK carries no date library.
 */
function relativePast(date: Date, now: Date): string {
  ...
  if (minutes < MINUTES_IN_DAY) return plural(Math.round(minutes / 60), 'hour');

  const dstNormalizedMinutes = (milliseconds - (zoneOffsetMs(now) - zoneOffsetMs(date))) / MINUTE;
  return plural(Math.round(dstNormalizedMinutes / MINUTES_IN_DAY), 'day');
}
 ...
 *
 * Strict distance (`relativePast`) rather than a loose one: it never rounds up
 * into a vague "about 1 minute", and the point of the label is a real
 * distance, not an impression.
 *
...
export function formatMessageDay(
  ...
  // Under a minute, a strict distance says "8 seconds ago" and then
  // "9 seconds ago" — motion that carries nothing. One word covers it.
  if (elapsed < MINUTE) return 'just now';
  if (elapsed < WEEK) return relativePast(date, new Date(now));
