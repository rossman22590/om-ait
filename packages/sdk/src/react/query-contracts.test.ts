...
describe("freshness contracts", () => {
  ...
  test("tiers are ordered from most to least fresh", () => {
    expect(contract("volatile").staleTime).toBeLessThan(
      contract("inventory").staleTime,
    );
    expect(contract("inventory").staleTime).toBeLessThan(
      contract("config").staleTime,
    );
    expect(contract("config").staleTime).toBeLessThan(
      contract("live").staleTime,
    );
  });
