// Migration: usage_events_request_id_unique_index  (NON-TRANSACTIONAL -- CONCURRENTLY escape hatch)
//
...
export const up = (pgm) => {
  ...
  // One row per LLM gateway request. `recordGatewayUsage` inserts with
  ...
  pgm.sql(`
    create unique index concurrently if not exists uniq_usage_events_request_id
      on kortix.usage_events (request_id)
