-- Migration: session_turn_end_error
--
...
-- IF NOT EXISTS on purpose: nullable, no default, no backfill — a metadata-only
-- change that is safe to re-run. Local stacks share one database across
-- worktrees, so the column can already exist when this branch's migration runs.
ALTER TABLE "kortix"."session_turns" ADD COLUMN IF NOT EXISTS "end_error" jsonb;