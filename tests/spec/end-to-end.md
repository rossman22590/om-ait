...
`SEC-POOL-2` Member grant and session pool → the member sees only granted keys; revocation hides them. The flag denies selection while off. Managers can select only keys also granted to the session owner. The session selects two keys; deletion prunes the IDs but preserves the configured empty pool in the pool listing. Resetting restores inherited behavior and removes the pool from that listing.
