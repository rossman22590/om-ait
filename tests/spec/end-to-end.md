...
`CONN-ACCOUNTS` One connector, several accounts: `GET /connectors/projects/:id/connectors/:slug/accounts` lists every connection the caller may reach — the caller's own private (`member`, `owner_id === caller`) accounts, and every project-owned (`member`) shared account, with the shared group's default account listed first within that group. A session-bound token and the human who owns its private account read the same list. `POST .../call` naming an `account` that does not resolve → `403 connector_not_connected` naming `requested_account` and `available_accounts` — never a silent substitution onto some other account. The same gate applies to the accounts list itself: narrowing `agents.kortix.connectors` off the connector → `403 connector_not_assigned` on both `GET .../accounts` and `POST .../call`.
