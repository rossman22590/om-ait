...
`SESS-33` A session pinned to an earlier `repository_generation` receives `409 {code:'session_repository_changed',remedy}` from ordinary `POST /projects/:projectId/sessions/:sessionId/start`. The owner can explicitly pass `repository_mode=previous` only while the session retains a sandbox `external_id`; the API then resumes that preserved workspace without changing the session generation, so the Git proxy continues to reject project Git access. A project member who does not own the session receives `403 {code:'previous_repository_resume_forbidden'}`. If the preserved runtime identity is gone, explicit previous mode returns `409 {code:'previous_repository_runtime_unavailable'}` and never provisions the current repository into the old session.
