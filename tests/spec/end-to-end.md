...
`CONN-26` Real agent provider-selection regression. A funded managed project boots a fresh Daytona session, sends the connector request through the live OpenCode proxy using `kortix/morph-glm53-744b`, and requires the agent to call its actual connector MCP tools. The assistant must return a `connect.composio.dev` authorization link and ask the user to open it. No tool call may set `provider:"pipedream"` or `allow_legacy_pipedream:true`; an explicit provider, when present, must be `composio`. Project REST readback must expose the agent-created Gmail connector with config `{provider:"composio",app:"gmail"}` and no Pipedream-backed Gmail declaration.
