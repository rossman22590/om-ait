...
Browser journey **28 — provider connection scope** keeps one provider row and one ChatGPT connect control. The existing project key field writes shared secrets. Selecting a personal key saves a user credential and binds it explicitly. Reload preserves the choice. Switching back preserves both stored credentials. A second project reuses the same personal connection without rewriting it. The subscription choice hides the API key field, and the row remains usable at mobile width. A stale connection fails visibly and preserves project mode. Gateway-disabled projects retain their original controls. Project readers can save and bind personal keys but cannot edit shared keys.
