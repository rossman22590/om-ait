...
`AGP-3` Flag OFF (default and explicit `false`). The same `reader` requests reproduce launcher ∩ grant: the member-launched run → 403 `project_role_insufficient` on files, the manager-launched run → 200 on files and 403 `agent_scope_insufficient` on secrets. The explicit override reads back `experimental.agent_principal: false` and gives identical results.
