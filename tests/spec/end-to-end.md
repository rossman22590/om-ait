...
ChatGPT usage regression: `GW-5` asserts explicit zero subscription rates and positive OpenAI API rates through the authenticated catalog. Browser journey 26 reads persisted historical transcripts and asserts `$0.00` for subscription usage, preserved token counts, and `$2.40` for a mixed session with `$2.00` raw API cost.
