...
`SEC-POOL-2` Member grant and session pool → the member sees only granted keys; revocation hides them. The flag denies selection while off. The session selects two keys, deleting one prunes only its ID, and resetting restores inherited behavior.
