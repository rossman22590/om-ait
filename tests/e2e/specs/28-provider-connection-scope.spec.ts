...
test.describe('28 — provider connection scope', () => {
  test('opens one connection dialog, commits explicit choices, and reuses personal credentials across projects', async ({ page }) => {
    ...
    try {
      ...
      page.on('request', r => {
        if (['POST', 'PUT', 'DELETE'].includes(r.method()) && /provider-connections|personal-providers|\/secrets$|\/oauth\//.test(r.url())) mutations.push(r.url());
      });
      ...
      await expect(dialog.getByRole('radio', { name: 'Use my account', exact: true })).toBeChecked();
      ...
      expect((await api<{ items: unknown[] }>(session.access_token, 'GET', '/provider-connections')).items).toEqual([]);
      ...
      expect(projectSaved.request().postDataJSON()).toMatchObject({ name: 'OPENAI_API_KEY', value: 'project-scope-ui-fixture', strategy: 'broker' });
      await expect(dialog).not.toBeVisible();
      await expect(row).toContainText('Connected · Project');

      await openConnection();
      await expect(dialog.getByRole('radio', { name: 'Share with this project', exact: true })).toBeChecked();
      ...
      expect((await api<{ items: unknown[] }>(session.access_token, 'GET', `/projects/${projects[0]}/personal-providers`)).items).toHaveLength(1);
      ...
      expect((await api<{ items: unknown[] }>(session.access_token, 'GET', `/projects/${projects[1]}/personal-providers`)).items).toEqual([]);
      await dialog.getByRole('button', { name: 'Cancel', exact: true }).click();
      await expect(row).toContainText('Not connected');
      ...
      const nativeSave = page.waitForResponse(r => r.request().method() === 'POST' && r.url().endsWith(`/projects/${native.id}/secrets`));
      await dialog.getByRole('button', { name: 'Save connection', exact: true }).click();
      ...
      await openConnection();
      await expect(dialog.getByRole('radio', { name: 'Use my account', exact: true })).toBeChecked();
      await expect(dialog.getByRole('radio', { name: 'Project connection', exact: true })).toBeDisabled();
      await chooseMethod('API key');
      ...
      await expect(dialog).not.toBeVisible();
      await expect(row).toContainText('Not connected');
      expect((await api<{ items: unknown[] }>(readerSession.access_token, 'GET', '/provider-connections')).items).toHaveLength(1);
