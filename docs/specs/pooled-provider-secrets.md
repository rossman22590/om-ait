...
- Put provider key and ChatGPT account management in Models → Providers, beside each provider. Show each named connection as a distinct row with its access count and actions. Keep general project secrets in Secrets. Keep `Remove from this session` separate from `Delete secret for everyone`.
