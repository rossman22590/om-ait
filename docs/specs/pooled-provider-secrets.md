...
- This release supports LLM gateway API keys, including Gemini keys through Google's OpenAI-compatible endpoint. OAuth credentials, runtime variables, and connector credentials require separate delivery adapters. ChatGPT Plus/Pro remains one project OAuth login; connecting another replaces the stored login.
