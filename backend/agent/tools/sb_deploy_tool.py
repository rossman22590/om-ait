...
class SandboxDeployTool(SandboxToolsBase):
    ...
    @openapi_schema({
        ...
        "function": {
            "name": "deploy",
            "description": "Deploy a static website (HTML+CSS+JS) from a directory in the sandbox to Cloudflare Pages. Only use this tool when permanent deployment to a production environment is needed. The directory path must be relative to /workspace. The website will be deployed to {name}.machine.cloud.",
            "parameters": {
                "type": "object",
                "properties": {
                    "name": {
