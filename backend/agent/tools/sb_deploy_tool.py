...
class SandboxDeployTool(SandboxToolsBase):
    ...
    def __init__(self, project_id: str, thread_manager: ThreadManager):
        ...
        logger.info(f"SandboxDeployTool initialized with project_id: {project_id}")
        logger.info(f"Cloudflare API token present: {bool(self.cloudflare_api_token)}")
    ...
    def _load_project_mappings(self):
        ...
        try:
            ...
            logger.warning(f"Could not load project mappings: {str(e)}")
            return {}
    ...
    def _save_project_mappings(self):
        ...
        try:
            if self.sandbox:
                ...
                logger.info(f"Saved project mappings to {self.mappings_file}")
        except Exception as e:
            logger.warning(f"Could not save project mappings: {str(e)}")
    ...
    def check_dns_record_exists(self, subdomain):
        ...
        if not all([self.cloudflare_api_token, self.cloudflare_zone_id]):
            logger.warning("Missing Cloudflare credentials for DNS check")
            return False
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"Error checking DNS records: {response.text}")
                return False
            ...
            logger.error(f"Exception checking DNS records: {str(e)}")
            return False
    
    def create_dns_record(self, subdomain, target):
        """Create a CNAME DNS record for the subdomain pointing to the Pages domain"""
        if not all([self.cloudflare_api_token, self.cloudflare_zone_id]):
            logger.warning("Missing Cloudflare credentials for DNS creation")
            return False
            
        if self.check_dns_record_exists(subdomain):
            logger.info(f"DNS record for {subdomain}.{self.custom_domain} already exists")
            return True
        ...
        logger.info(f"Creating DNS record: {subdomain}.{self.custom_domain} -> {target}")
        try:
            response = requests.post(url, headers=self.cf_headers, json=payload)
            
            if response.status_code == 200:
                logger.info(f"Successfully created DNS record for {subdomain}.{self.custom_domain}")
                return True
            else:
                logger.error(f"Failed to create DNS record: {response.text}")
                return False
        except Exception as e:
            logger.error(f"Exception creating DNS record: {str(e)}")
            return False
    ...
    def assign_custom_domain_to_pages(self, project_name, subdomain):
        ...
        if not all([self.cloudflare_api_token, self.cloudflare_account_id]):
            logger.warning("Missing Cloudflare credentials for domain assignment")
            return False
        ...
        try:
            ...
            if check_response.status_code == 200:
                ...
                if full_domain in domain_names:
                    logger.info(f"Domain {full_domain} is already assigned to Pages project: {project_name}")
                    return True
        except Exception as e:
            logger.error(f"Error checking existing domains: {str(e)}")
        ...
        full_domain = f"{subdomain}.{self.custom_domain}"
        logger.info(f"Assigning custom domain {full_domain} to Pages project: {project_name}")
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"Successfully assigned domain {full_domain} to Pages project: {project_name}")
                # After custom domain assignment, need to wait a bit for verification to complete
                time.sleep(2)
                return True
            else:
                logger.error(f"Failed to assign domain to Pages project {project_name}: {response.text}")
                return False
        except Exception as e:
    ...
    def deploy(self, name: str, directory_path: str, setup_custom_domain: bool = True) -> ToolResult:
        ...
        try:
            ...
            logger.info(f"Cleaned path: {full_path}")
            
            # Verify the directory exists
            try:
                logger.info(f"Checking if directory exists: {full_path}")
                ...
                logger.info(f"Directory exists and is valid: {full_path}")
            except Exception as e:
                logger.error(f"Directory check failed: {str(e)}", exc_info=True)
            ...
            try:
                ...
                logger.info("Cloudflare API token is set")
                ...
                if name in self.project_mappings:
                    ...
                    logger.info(f"Reusing existing project name {project_name} for {name}")
                ...
                if len(project_name) > 58:
                    logger.error(f"Project name too long: {project_name}")
                    return self.fail_response(
                ...
                if project_exists:
                    logger.info(f"Project exists, deploying directly: {project_name}")
                    ...
                    logger.info(f"Project doesn't exist, creating first: {project_name}")
                ...
                try:
                    ...
                    logger.info(f"Command executed with exit code: {response.exit_code}")
                    logger.info(f"Command output: {response.result}")
                except Exception as exec_error:
                    logger.error(f"Command execution failed: {str(exec_error)}", exc_info=True)
                    return self.fail_response(f"Command execution failed: {str(exec_error)}")
                ...
                if response.exit_code == 0:
                    ...
                    logger.error(f"Deployment failed with exit code {response.exit_code}: {response.result}")
