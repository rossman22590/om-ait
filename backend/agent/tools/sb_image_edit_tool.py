        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                try:
                    ...
                    try:
                        # Fix argument order: content first, then path
                        self.sandbox.fs.upload_file(image_bytes, image_path)
                        logging.info(f"Edited image saved to: {image_path}")
                    except Exception as e:
                        logging.error(f"Error saving edited image to sandbox: {str(e)}")
                        return ToolResult(
                            success=False,
                            output=f"Error saving edited image to workspace: {str(e)}"
                        )
                    ...
                    try:
                        ...
                        logging.info(f"Uploading edited image to Pixio media using multipart/form-data")
                        ...
                        async with httpx.AsyncClient(timeout=60.0) as upload_client:
                            ...
                            logging.info(f"Edited image uploaded successfully to Pixio media: {public_url}")
                            
                            # Return success with URL
                            return ToolResult(
                                success=True,
                                output=f"Image successfully edited and saved to workspace at: images/{filename}\n\nImage URL: {public_url}?t={timestamp}"
                            )
                    except Exception as e:
                        logging.error(f"Error uploading edited image to Pixio media: {str(e)}")
                        # Fall back to just returning the path if upload fails
                        return ToolResult(
                            success=True,
                            output=f"Image successfully edited and saved to workspace at: images/{filename}?t={timestamp}\n\nNote: External upload failed, but image is available in workspace."
                        )
