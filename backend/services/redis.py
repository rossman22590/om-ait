...
# async def publish(channel: str, message: str, retries: int = 2):
...
#             if attempt > 0:
#                 logger.debug(f"Successfully published to {channel} after {attempt+1} attempts")
#             return result
#         except Exception as e:
#             if attempt < retries:
#                 logger.warning(f"Redis publish to {channel} failed (attempt {attempt+1}): {str(e)}. Retrying...")
#                 await asyncio.sleep(0.1 * (attempt + 1))  # Increasing backoff
#             else:
#                 logger.error(f"Redis publish to {channel} failed after {retries+1} attempts: {str(e)}")
#                 # On final failure, try to re-initialize the connection
#                 try:
#                     await initialize_async()
#                     # One final attempt after re-initialization
#                     return await redis_client.publish(channel, message)
#                 except Exception as reinit_error:
#                     logger.error(f"Redis re-initialization failed: {str(reinit_error)}")
#                     raise e
