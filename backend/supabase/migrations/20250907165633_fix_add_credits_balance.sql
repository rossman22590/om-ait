...
BEGIN
    ...
    ELSIF v_column_name = 'account_id' THEN
        IF p_is_expiring THEN
            INSERT INTO credit_accounts (account_id, expiring_credits, non_expiring_credits, balance, tier)
            VALUES (p_user_id, p_amount, 0, p_amount, 'free')
        ...
        INSERT INTO credit_ledger (
            account_id, amount, balance_after, type, description, is_expiring, expires_at
        )
        VALUES (
            p_user_id, p_amount, v_new_total,
            CASE WHEN p_is_expiring THEN 'tier_grant' ELSE 'purchase' END,
