...
BEGIN
    ...
    IF v_column_name = 'user_id' THEN
        -- Create temp function for user_id
        v_sql := '
        CREATE OR REPLACE FUNCTION temp_calculate_credit_split(p_user_id UUID)
        RETURNS TABLE(expiring_amount DECIMAL, non_expiring_amount DECIMAL) AS $func$
        DECLARE
    ...
    ELSIF v_column_name = 'account_id' THEN
        -- Create temp function for account_id
        v_sql := '
        CREATE OR REPLACE FUNCTION temp_calculate_credit_split(p_user_id UUID)
        RETURNS TABLE(expiring_amount DECIMAL, non_expiring_amount DECIMAL) AS $func$
        DECLARE
        ...
        -- Process backfill for account_id
        DECLARE
            r RECORD;
            v_expiring DECIMAL;
            v_non_expiring DECIMAL;
        ...
        BEGIN
            ...
            LOOP
                SELECT expiring_amount, non_expiring_amount
                INTO v_expiring, v_non_expiring
                FROM temp_calculate_credit_split(r.account_id);
