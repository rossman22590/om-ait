...
async def run_agent_background(
    ...
    # Resolve effective model considering agent_config and known aliases
    effective_model = model_name
    try:
        if model_name == "anthropic/claude-sonnet-4-20250514" and agent_config and agent_config.get('model'):
            # If user didn't explicitly choose a model (default is Claude), use the agent's configured model
            ...
            # Map short/legacy names to fully-qualified provider-prefixed names
            from utils.constants import MODEL_NAME_ALIASES
        ...
        logger.warning(f"Model resolution failed for '{model_name}', falling back to provided name. Error: {model_err}")
        effective_model = model_name
