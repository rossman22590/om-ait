...
def _redis_broker_url() -> str:
    """Build a Redis URL for Dramatiq RedisBroker - Railway Redis Edition.
    ...
    - REDIS_SSL="false"
    """
    # Check for direct Railway REDIS_URL first
    url = os.getenv('REDIS_URL')
    if url:
        logger.info(f"Dramatiq using Railway REDIS_URL: {url.split('@')[-1] if '@' in url else url}")
        return url
    ...
    provider = os.getenv('REDIS_PROVIDER', 'railway')
    ...
    built_url = f"{scheme}://{auth}{host}:{port}"
    logger.info(f"Dramatiq built Railway Redis URL: provider={provider}, host={host}:{port}, ssl={use_ssl}")
    logger.info(f"Dramatiq Railway Redis: {scheme}://default:***@{host}:{port}")
...
async def run_agent_background(
    ...
    """Background task to run an agent with Railway Redis support."""
    logger.info(f"[RAILWAY WORKER] Starting background agent run for thread {thread_id}")
    
    # Set up Sentry context
    sentry_sdk.set_tag("thread_id", thread_id)
    sentry_sdk.set_tag("project_id", project_id)
    sentry_sdk.set_tag("agent_run_id", agent_run_id)
    
    try:
        # Run the agent with matching parameters
        ...
        logger.info(f"[RAILWAY WORKER] Background agent run completed successfully for thread {thread_id}")
        return result
        
    except Exception as e:
        logger.error(f"[RAILWAY WORKER] Background agent run failed for thread {thread_id}: {e}")
        logger.error(f"[RAILWAY WORKER] Traceback: {traceback.format_exc()}")
        ...
        try:
            asyncio.run(_update_run_status(agent_run_id, 'failed', str(e)))
        except Exception as update_error:
            logger.error(f"[RAILWAY WORKER] Failed to update run status: {update_error}")
        
        raise

async def _update_run_status(agent_run_id: str, status: str, error_message: Optional[str] = None):
    ...
    try:
        ...
        logger.info(f"[RAILWAY WORKER] Updated run {agent_run_id} status to {status}")
        
    except Exception as e:
        logger.error(f"[RAILWAY WORKER] Failed to update run status for {agent_run_id}: {e}")
...
def enqueue_agent_run(
    ...
    """Enqueue an agent run for background processing on Railway."""
    
    logger.info(f"[RAILWAY WORKER] Enqueuing background agent run {agent_run_id} for thread {thread_id}")
    
    # Send task to Railway Redis queue
    ...
    logger.info(f"[RAILWAY WORKER] Enqueued background agent run {agent_run_id}")
    return agent_run_id

if __name__ == "__main__":
    logger.info("[RAILWAY WORKER] Starting Railway Redis background worker")
    logger.info(f"[RAILWAY WORKER] Redis URL: {_redis_broker_url().split('@')[-1] if '@' in _redis_broker_url() else _redis_broker_url()}")
    
    # Start the worker
    dramatiq.cli.main([
        "run_agent_background_railway",
        "--processes", "4",
        "--threads", "8"
    ])
