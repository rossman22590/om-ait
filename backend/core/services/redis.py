...
def _build_redis_url() -> tuple[str, bool]:
    """Build a Redis URL for Railway Redis.
    
    Railway Redis uses format: redis://default:password@host:port
    ...
    # Check for direct REDIS_URL first (Railway provides this as {{Redis.REDIS_PRIVATE_URL}})
    url = os.getenv("REDIS_URL")
    if url:
        ssl_on = url.startswith("rediss://")
        logger.info(f"Using Railway REDIS_URL from {{{{Redis.REDIS_PRIVATE_URL}}}}: {url.split('@')[-1] if '@' in url else url}")
        return url, ssl_on

    # Build from Railway environment variables
    # Railway sets these as: {{Redis.RAILWAY_PRIVATE_DOMAIN}}, {{Redis.REDIS_PASSWORD}}
    ...
    # Railway Redis is typically non-SSL
    ...
    logger.info(f"Built Railway Redis URL from Railway vars: provider={provider}, host={host}:{port}, ssl={use_ssl}")
    logger.info(f"Railway Redis URL: {scheme}://default:***@{host}:{port}")
    return built, use_ssl


def initialize():
    """Initialize Redis client for Railway Redis."""
    global client, pool

    # Connection options - optimized for Railway Redis
    max_connections = int(os.getenv("REDIS_MAX_CONNECTIONS", "512"))
    ...
    try:
        logger.info(f"Initializing Railway Redis client: ssl={ssl_on} max={max_connections}")
        ...
        # Use connection pool via from_url for Railway Redis
        ...
        logger.error(f"Failed to initialize Railway Redis client: {e}")
        raise


async def initialize_async():
    """Initialize Railway Redis connection asynchronously."""
    ...
    async with _init_lock:
        ...
        try:
            ...
            logger.info("Successfully connected to Railway Redis")
            _initialized = True
        except asyncio.TimeoutError:
            logger.error("Railway Redis connection timeout during initialization")
            client = None
            _initialized = False
            raise ConnectionError("Railway Redis connection timeout")
        except Exception as e:
            logger.error(f"Failed to connect to Railway Redis: {e}")
            client = None
            _initialized = False
            raise
...
async def close():
    """Close Railway Redis connection and connection pool."""
    global client, pool, _initialized
    if client:
        logger.info("Closing Railway Redis connection")
        try:
            await asyncio.wait_for(client.aclose(), timeout=5.0)
        except asyncio.TimeoutError:
            logger.warning("Railway Redis close timeout, forcing close")
        except Exception as e:
            logger.warning(f"Error closing Railway Redis client: {e}")
        finally:
            client = None
    
    if pool:
        logger.info("Closing Railway Redis connection pool")
        try:
            await asyncio.wait_for(pool.aclose(), timeout=5.0)
        except asyncio.TimeoutError:
            logger.warning("Railway Redis pool close timeout, forcing close")
        except Exception as e:
            logger.warning(f"Error closing Railway Redis pool: {e}")
        finally:
            pool = None
    
    _initialized = False
    logger.info("Railway Redis connection and pool closed")


async def get_client():
    """Get the Railway Redis client, initializing if necessary."""
    global client, _initialized
