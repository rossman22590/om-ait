...
export async function generateMetadata({ params }: { params: Promise<{ shareId: string }> }): Promise<Metadata> {
  ...
  try {
    const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL || 'http://localhost:8000'}/templates/public/${templateId}`);
    ...
    return {
      title: 'AI Agent Template | Machine',
