...
export const useModelSelection = () => {
  ...
  // Auto-switch to best model when user upgrades to paid tier
  useEffect(() => {
    ...
    if (wasFree && isNowPaid && prevTierKey.current !== null) {
      // Switch to the top accessible model (sorted by priority)
      const topModel = availableModels.find(m => !m.requiresSubscription);
      if (topModel) {
        console.log('🚀 useModelSelection: Upgraded to paid tier! Switching to:', topModel.id);
        setSelectedModel(topModel.id);
      }
    }
