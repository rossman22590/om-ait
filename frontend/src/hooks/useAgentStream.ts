...
export function useAgentStream(
  ...
  const finalizeStream = useCallback(
    (finalStatus: string, runId: string | null = agentRunId) => {
      ...
      console.log(
        `[useAgentStream] Stream finalized with status: ${finalStatus}`,
      );

      // Dispatch custom events to notify ThreadTimer component about status change
      if (typeof window !== 'undefined' && threadId) {
        try {
          // Thread-specific event (already existing)
          const threadSpecificEvent = new CustomEvent(`agent-status-change-${threadId}`, {
            detail: {
              status: finalStatus,
              agentRunId: runId,
              threadId: threadId,
          ...
          // Generic event for all listeners (new)
          const genericEvent = new CustomEvent('agent-status-change', {
            detail: {
              status: finalStatus,
              agentRunId: runId,
              threadId: threadId,
          ...
          console.log(`[useAgentStream] Dispatching status change events for ${threadId}:`, finalStatus);
          window.dispatchEvent(threadSpecificEvent);
          window.dispatchEvent(genericEvent); // Dispatch generic event
        } catch (e) {
          console.error('[useAgentStream] Error dispatching status events:', e);
        }
      }

      // Retrieve the full messages list for this thread after we finalize
      if (finalStatus === 'completed' || finalStatus === 'stopped' || finalStatus === 'failed') {
        if (currentThreadId) {
          console.log(
          ...
          console.log(
            `[useAgentStream] Skipping message refetch for unknown thread. Final status: ${finalStatus}`,
          );
        }
        ...
        if (
          ...
          (finalStatus === 'completed' ||
            finalStatus === 'stopped' ||
            finalStatus as string === 'agent_not_running')
        ) {
