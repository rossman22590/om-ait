...
const DotGrid: React.FC<DotGridProps> = ({
  ...
  const effectiveBaseColor = useMemo(() => {
    if (currentTheme === 'dark') {
      return baseColor; // Use the provided baseColor for dark mode
    } else {
      return '#5227FF'; // Purple for light mode
    }
  }, [currentTheme, baseColor]);

  const effectiveActiveColor = useMemo(() => {
    if (currentTheme === 'dark') {
      return activeColor; // Use the provided activeColor for dark mode
    } else {
      return '#5227FF'; // Purple for light mode
    }
